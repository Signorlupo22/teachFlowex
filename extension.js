...
function activate(context) {
    ...
    if (!workspaceFolders) {
        vscode.window.showErrorMessage('Nessuna cartella aperta nel workspace.');
        return;
    }
    ...
    let startAuthCommand = vscode.commands.registerCommand('devlern.startAuth', async () => {
        ...
        try {
            ...
            app.get('/callback', async (req, res) => {
                ...
                const decoded = jwt ? jwt.decode(jwtToken.toString()) : null;
                if ( decoded) {
                    ...
                    vscode.window.showInformationMessage('Trying to connect to WebSocket');
                    connectToWebSocket();
                    server.close();
                } else {
            ...
            const authUrl = 'http://localhost:3000/api/Extension?token=' + userInput;
            opn(authUrl);
    ...
    function decodeResponse(data) {
        ...
        switch (json.type) {
            case "gitClone":
                exec(`git clone ${json.url} .`, { cwd: currentDir }, (error, stdout, stderr) => {
                    if (error) {
                ...
                break;
            case "insertCode":
                const [line, column] = json.lineAndColumn.split(":").map(Number);
                const filePath = path.join(currentDir, json.file);
                const fileUri = vscode.Uri.file(filePath);
                vscode.workspace.openTextDocument(fileUri).then(document => {
                    vscode.window.showTextDocument(document).then(editor => {
                        const position = new vscode.Position(line - 1, column - 1); // Le linee e colonne iniziano da 0
                        editor.edit(editBuilder => {
                            editBuilder.insert(position, json.code);
                        }).then(success => {
                            if (success) {
                                vscode.window.showInformationMessage(`Codice inserito in ${json.file} alla posizione ${json.lineAndColumn}`);
                            } else {
                                vscode.window.showErrorMessage('Errore durante l\'inserimento del codice.');
                            }
                        });
                    });
                }, error => {
            ...
            case "removeLines":
                ...
                vscode.workspace.openTextDocument(removeFileUri).then(document => {
                    vscode.window.showTextDocument(document).then(editor => {
                        const startLine = json.startLine - 1; // Le linee inizia da 0
                        const endLine = json.endLine ? json.endLine - 1 : startLine; // Se endLine non è fornito, rimuove una sola linea
                        ...
                        editor.edit(editBuilder => {
                            ...
                            if (success) {
                                vscode.window.showInformationMessage(`Linee ${json.startLine} - ${json.endLine || json.startLine} rimosse in ${json.file}`);
                            } else {
                                vscode.window.showErrorMessage('Errore durante la rimozione delle linee.');
                            }
                        });
                    });
                }, error => {
